...
function ShootingStars() {
  ...
  return (
    <lineSegments ref={linesRef} geometry={geometry}>
      <lineBasicMaterial color="#00F0FF" transparent opacity={0.6} blending={THREE.AdditiveBlending} />
    </lineSegments>
  );
}
...
function AnimatedPlanet() {
  ...
  useFrame((state, delta) => {
    ...
    currentSpinSpeed.current = THREE.MathUtils.damp(currentSpinSpeed.current, targetSpinSpeed, 4, delta);
    ...
    groupRef.current.rotation.x = THREE.MathUtils.damp(groupRef.current.rotation.x, targetTiltX, 4, delta);
    groupRef.current.rotation.z = THREE.MathUtils.damp(groupRef.current.rotation.z, targetTiltZ, 4, delta);
    ...
    groupRef.current.position.y = THREE.MathUtils.damp(groupRef.current.position.y, targetYPos, 6, delta);
...
function Scene() {
  return (
    <>
      ...
      <directionalLight position={[-10, -10, -5]} intensity={1.5} color="#B200FF" />
